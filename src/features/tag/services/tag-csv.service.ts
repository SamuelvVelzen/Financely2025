...
export function autoDetectTagMapping(
  columns: string[]
): Partial<ITagCsvFieldMapping> {
  const mapping: Partial<ITagCsvFieldMapping> = {};
 ...
 */
export function validateTagMapping(mapping: ITagCsvFieldMapping): {
  valid: boolean;
...
): Partial<ICreateTagInput> {
  const tag: Partial<ICreateTagInput> = {};
...
export function convertRowsToTagCandidates(
  ...
  const candidates: ITagCsvCandidate[] = [];
...
export { MAX_FILE_SIZE, MAX_ROWS, parseCsvHeaders, parseCsvRows };
